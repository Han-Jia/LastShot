...
class ResNetDeeper(nn.Module):
    ...
    def __init__(self, block=BasicBlock, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=10):
        self.inplanes = 3
        super(ResNetDeeper, self).__init__()

        self.layer1 = self._make_layer(block, 64, stride=1, drop_rate=drop_rate)
        self.layer2 = self._make_layer(block, 64, stride=1, drop_rate=drop_rate)
        self.layer3 = self._make_layer(block, 160, stride=2, drop_rate=drop_rate)        
        self.layer4 = self._make_layer(block, 320, stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        self.layer5 = self._make_layer(block, 320, stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
        self.layer6 = self._make_layer(block, 640, stride=2, drop_rate=drop_rate, drop_block=True, block_size=dropblock_size)
