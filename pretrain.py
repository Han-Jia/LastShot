...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='CUB', choices=['MiniImageNet', 'TieredImagenet', 'TieredImagenetRaw', 'CUB', 'CIFARFS', 'FC100'])
    parser.add_argument('--backbone_class', type=str, default='Res101', 
                        choices=['Res12', 'Res18', 'Res50', 'Res101', 'Res152'])
    ...
    save_path2 = '_'.join([str(args.lr), str(args.gamma), str(args.schedule), str(args.decay)])
    ...
    if args.dataset == 'MiniImageNet':
        ...
        args.dropblock_size = 5
    elif args.dataset == 'TieredImagenetRaw':
    ...
    train_loader = DataLoader(dataset=trainset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
    ...
    if 'Conv' in  args.backbone_class:
        ...
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9, nesterov=True, weight_decay=args.decay)
    else:
    ...
    for epoch in range(init_epoch, args.max_epoch + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            if torch.cuda.is_available():
                data, label = batch[-2].cuda(), batch[-1].cuda()
                label = label.type(torch.cuda.LongTensor)
            else:
                data, label = batch[-2], batch[-1]
